...
import com.example.tahuuduc_duan1_admin.adapter.LoaiSPAdapter;
...
public class DanhSachLoaiSPFragment extends Fragment implements OnClickItem {
    ...
    private void saveAndGetImgLinkWhenAdd(LoaiSP loaiSP, Uri imgUri) {
    ...
    private boolean validSPWhenAdd(LoaiSP loaiSP, List<LoaiSP> loaiSPList) {
    ...
    private void openDialogSuaLoaiSP(LoaiSP loaiSP) {
        ...
        Picasso.get().load(loaiSP.getHinhanh())
    ...
    private void saveAndGetImgLinkWhenEdit(LoaiSP loaiSP, Uri imgUri) {
        FirebaseStorage firebaseStorage = FirebaseStorage.getInstance();
        String strImgUri = loaiSP.getName() + "*" + System.currentTimeMillis() + "." + OverUtils.getExtensionFile(getContext(), imgUri);
        StorageReference stRef = firebaseStorage.getReference("image/loai_sp").child(strImgUri);
        stRef.putFile(imgUri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
    ...
    private boolean validSPWhenEdit(LoaiSP loaiSP, List<LoaiSP> loaiSPList) {
        for (LoaiSP lSp : loaiSPList) {
            if (lSp.getId().equals(loaiSP.getId())) {
                continue;
            }
            if (lSp.getName().equals(loaiSP.getId())) {
                dgEditLoading.setVisibility(View.INVISIBLE);
                OverUtils.makeToast(getContext(), "Đã tồn tại loại sp này trong hệ thống");
                return false;
            }
    ...
    public void onDeleteItem(Object obj) {
        ...
        if (loaiSP != null) {
            new AlertDialog.Builder(getContext())
                    ...
                    .setMessage("Bạn có chắc chắn muốn xóa?\n Bạn sẽ xóa luôn cả sản phẩm thuộc loại!")
    ...
    private void xoaDonHang(List<Product> productList) {
    ...
    private void ngungKinhDoanhPr(List<Product> productList, IDone iDone) {
